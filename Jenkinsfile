  ...
  stages {
    ...
    stage('Print Access Info') {
      steps {
        ...
        bat """
          powershell -NoProfile -Command "\$ip = (minikube ip).Trim(); \$host = 'app.' + \$ip + '.nip.io'; Write-Output '==================================='; Write-Output '  ACCESS YOUR APPLICATION HERE:'; Write-Output ('      http://' + \$host + '/'); Write-Output '==================================='; try { Invoke-WebRequest -UseBasicParsing -Uri ('http://' + \$host + '/') -TimeoutSec 5 | Out-Null; Write-Output 'Frontend reachable ✔'; } catch { Write-Warning 'Frontend NOT reachable'; }"
        """
      }
    }
  }
