  ...
  stages {
    ...
    stage('Print Access Info') {
      steps {
        ...
        bat """
          powershell -NoProfile -Command "\$ip = (minikube ip).Trim(); \$appHost = 'app.' + \$ip + '.nip.io'; Write-Output '==================================='; Write-Output '  ACCESS YOUR APPLICATION HERE:'; Write-Output ('      http://' + \$appHost + '/'); Write-Output '==================================='; try { Invoke-WebRequest -UseBasicParsing -Uri ('http://' + \$appHost + '/') -TimeoutSec 5 | Out-Null; Write-Output 'Frontend reachable ✔'; } catch { Write-Warning 'Frontend NOT reachable ❌'; }"
        """
      }
    }
  }
